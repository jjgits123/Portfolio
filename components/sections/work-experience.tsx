...
export default function WorkExperience() {
  ...
  const workExperiences = [
    {
      company: "Procter & Gamble Philippines Business Services, Inc.",
      location: "Taguig, Philippines",
      role: "Master Data Specialist",
      duration: "Sep 2025 - Dec 2025",
      logo: "/images/company-services/pg-logo.png",
      responsibilities: [
        {
          icon: <Database className="h-5 w-5 text-purple-400" />,
          text: "Developed ETL pipelines using Azure Databricks to standardize and transform global and local datasets, improving data accuracy and consistency.",
        },
        {
          icon: <Zap className="h-5 w-5 text-purple-400" />,
          text: "Implemented rule-based automation for key product attributes, achieving 95%+ accuracy in high-derivability categories.",
        },
        {
          icon: <CheckCircle2 className="h-5 w-5 text-purple-400" />,
          text: "Conducted data validation and quality assurance, identifying discrepancies and ensuring alignment with internal standards.",
        },
        {
          icon: <FileText className="h-5 w-5 text-purple-400" />,
          text: "Documented processes and created a rule-to-code traceability matrix, supporting maintainability and reproducibility of automated workflows.",
        },
      ],
      technologies: ["Azure Databricks", "PowerBi", "Sharepoint", "Pyspark programming", "ETL Pipeline", "SAP"],
    },
    {
      ...
      <div className="space-y-12 max-w-5xl mx-auto">
        {workExperiences.map((experience, expIndex) => (
          <div
            ...
            style={{
              transitionDelay: mounted ? `${expIndex * 300}ms` : "0ms",
            }}
          >
            {/* Company Header */}
            <div className="flex flex-col md:flex-row items-start md:items-center gap-6 mb-8">
              ...
              <div className="flex-grow">
                <h3 className="text-2xl font-bold text-gray-200 mb-2">{experience.role}</h3>
              ...
              <div className="grid grid-cols-1 gap-4">
                {experience.responsibilities.map((responsibility, index) => (
                  <div
                    ...
                    style={{
                      transitionDelay: mounted ? `${expIndex * 300 + (index + 1) * 150}ms` : "0ms",
                    }}
                  >
                    <div className="flex-shrink-0 mt-1">{responsibility.icon}</div>
                    <p className="text-gray-300 leading-relaxed text-base md:text-lg">{responsibility.text}</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {experience.technologies.map((tech, index) => (
                  <span
                    ...
                    style={{
                      transitionDelay: mounted ? `${expIndex * 300 + 800 + index * 50}ms` : "0ms",
                    }}
                  >
                    {tech}
                  </span>
